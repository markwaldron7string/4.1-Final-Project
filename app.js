...
const calorieLimit = calorieSlider.value;
...
query)}&number=5&addRecipeNutrition=true&maxCalories=${calorieLimit}&apiKey=${apiKey}`;
...
if (calInfo)
calories = Math.round(calInfo.amount) + " " + calInfo.unit;
if (proteinInfo)
protein = Math.round(proteinInfo.amount) + " " + proteinInfo.unit;
if (fatInfo) fat = Math.round(fatInfo.amount) + " " + fatInfo.unit;
if (carbsInfo)
carbs = Math.round(carbsInfo.amount) + " " + carbsInfo.unit;
        }
...
resultsDiv.innerHTML =
'<div class="error">Error fetching recipes. Please try again later.</div>';
    });
}
